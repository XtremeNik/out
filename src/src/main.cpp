...
#if defined(NDEBUG)
# error "Kin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x23839343632538d86a8382379dc02f3d5e415c40a2ebf3e229b2bf5fc861ff3b");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Kin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Kin Signed Message:\n";
...
{
    // Kin: IsDust() detection disabled, allows any valid dust to be relayed.
...
{
    int64 nSubsidy = 70 * COIN;

    // Subsidy is cut in half every 60953 blocks, which will occur (goal) approximately every 2 years
    nSubsidy >>= (nHeight / 60953); // Kin: Goal: 61k blocks in ~2 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // Kin: 1 days 3.5
static const int64 nTargetSpacing = 5 * 60; // Kin: 5 minutes 2.5
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Kin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Kin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xefbb6905f549d833fff111c5579af337a01534b4e28c585b9fcc5b35dd378aaf");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Launch of MEI 1/27/2018 big plans ahead. Hodl we will profit";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 70 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04cf0b39a844c6f86bd02cbdc6eaeaa8:d4f6bc5bc185d20fd07464887db78426579c0d4596363a969053d6c110e5374dc80e70089bbdca94701c36b12c86b43a74") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1517049646;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2085069138;

        if (fTestNet)
        {
            block.nTime    = 1517049630;
            block.nNonce   = 386288776;
        }
...
if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0xf2087e657c9fe88daee7b3c8b7ee304272c7ffb8239587e041f9e04ed31fc390"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfa, 0xc3, 0xba, 0xd3 }; // Kin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("KinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("KinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static KinMiner(CWallet *pwallet)
{
    printf("KinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("kin-miner");
    ...
    try { loop {
        ...
        printf("Running KinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&KinMiner, pwallet));
}
